...
export const projectsAPI = {
  async getAll(userId: string): Promise<Project[]> {
    ...
    // Admins see all projects, users see only their projects
    if (user.role === "ADMIN") {
...
export const tasksAPI = {
  ...
  async create(
    ...
    if (data.assigneeId) {
      if (!isUserProjectMember(data.assigneeId, data.projectId)) {
  ...
  async update(
    ...
    db.addAuditLog({
      ...
      entity: "TASK",
      entityId: taskId,
      before: { title: before.title, priority: before.priority, assigneeId: before.assigneeId },
      after: { title: updatedTask.title, priority: updatedTask.priority, assigneeId: updatedTask.assigneeId },
    });
  ...
  async changeStatus(
    ...
    // Check if status change is allowed for this user
  ...
  async delete(taskId: string, userId: string): Promise<void> {
    ...
    if (!permissions.canDeleteTask) {
      throw new APIError("Only admins can delete tasks", 403);
    }
...
export const auditAPI = {
  async getAll(userId: string): Promise<AuditLogEntry[]> {
    ...
    if (!permissions.canViewAuditLog) {
      throw new APIError("Only admins can view audit logs", 403);
    }
